...
public class MiniMaxGameBlack {
    ...
    public static void main(String[] args) throws IOException {
        ...
        char[] swapped_out_board = minimaxgameblack.MAXIMUM_MINIMUM(swapped_input, depth);
        char[] outboard = FUNCTION_SWAPBOARD(swapped_out_board);
        System.out.println("Board Position: " + new String(outboard));
        System.out.println("Positions evaluated by static estimation: " + minimaxgameblack.position);
        System.out.println("MINIMAX estimate: " + - minimaxgameblack.approx_pos);
        file_output.println("Board Position : " + new String(outboard));
        file_output.println("Positions evaluated by static estimation : " + minimaxgameblack.position);
        file_output.println("MiniMax estimate : " + - minimaxgameblack.approx_pos);
        input.close();
        file_output.close();
    }
    ...
    public int STATIC_ESTIMATE(char[] board) {
        int w = 0;
        int b = 0;
        ArrayList < char[] > black_move_list = FUNCTION_MOVE_BLACK(board);
        int black_count = black_move_list.size();
        for (char c : board) {
            if (c == 'W') {
                w++;
            } else if (c == 'B') {
        ...
        if (b <= 2) {
            ...
            return ((1000 * (w - b)) - black_count);
        }
    }

    private ArrayList < char[] > FUNCTION_GEN_MOVE(char[] board) {
        ...
        if (w == 3) {
            board_position = FUNCTION_HOP(board);
        } else {
            board_position = FUNCTION_MOVE(board);
        }
    ...
    public ArrayList < char[] > FUNCTION_MOVE_BLACK(char[] x) {
        ...
        black_move = FUNCTION_GEN_MOVE(board_swap);
