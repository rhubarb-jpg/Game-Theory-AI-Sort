...
public class MiniMaxGame{
    ...
    public static void main(String[] args) throws IOException {
        ...
        char[] board_output = minimaxgame.MAXIMUM_MINIMUM(board_input, depth);
        System.out.println("Board Position: " + new String(board_output));
        System.out.println("Positions evaluated by static estimation: " + minimaxgame.position);
        System.out.println("MINIMAX estimate: " + minimaxgame.approx_pos);
        file_output.println("Board Position : " + new String(board_output));
        file_output.println("Positions evaluated by static estimation : " + minimaxgame.position);
        file_output.println("MiniMax estimate : " + minimaxgame.approx_pos);
        input.close();
        file_output.close();
    }
//Passes character and board (x)
    ...
    public int STATIC_ESTIMATE_CONTINUED(char[] x) {
        int w = 0;
        int b = 0;
        ArrayList < char[] > black_Moves = FUNCTION_MOVE_BLACK(x);
        int count = black_Moves.size();
        for (char c : x) {
            if (c == 'W') {
                w++;
            } else if (c == 'B') {
